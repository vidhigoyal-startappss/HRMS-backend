...
export class AttendanceController {
  ...
  async checkIn(@User() user: JwtPayload, @Body() dto: CheckInDto) {
  ...
  @Get("all")
  @Permissions({ resource: "attendance", action: "read" })
  @ApiOkResponse({ description: "All attendance history for all users" })
  async getAllAttendance(@User() user: JwtPayload) {
    return this.attendanceService.getAllAttendanceHistory(user);
  }
}
